<? /* Upgrade from version 0.11 to 0.12
...
if ("0.11." != substr($oldversion, 0, 5).'.') {
    die("Can't upgrade from 0.11.x, since the current db version is {$oldversion}.");
}

$db = new DBConnection();
$db->beginTransaction();
$q = $db->prepare("ALTER TABLE `{$db->getPrefix()}blocks`
    ADD COLUMN `weeklygradual` boolean default TRUE AFTER `notes`");
