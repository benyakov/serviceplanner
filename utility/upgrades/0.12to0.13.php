<? /* Upgrade from version 0.12 to 0.13
    Copyright (C) 2015 Jesse Jacobsen
...
if ("0.12." != substr($oldversion, 0, 5).'.') {
    die("Can't upgrade from 0.12.x, since the current db version is {$oldversion}.");
}

$db = new DBConnection();
$db->beginTransaction();
$q = $db->prepare("ALTER TABLE `{$db->getPrefix()}churchyear_lessons`
    ADD COLUMN `note` text AFTER `hymn`");
$q->execute() or die(array_pop($q->errorInfo()));
$q = $db->prepare("ALTER TABLE `{$db->getPrefix()}churchyear_lessons`
    MODIFY COLUMN `hymnabc` VARCHAR(80)");
$q->execute() or die(array_pop($q->errorInfo()));
$q = $db->prepare("ALTER TABLE `{$db->getPrefix()}churchyear_lessons`
    MODIFY COLUMN `hymn` VARCHAR(80)");
$q->execute() or die(array_pop($q->errorInfo()));
$db->commit();
