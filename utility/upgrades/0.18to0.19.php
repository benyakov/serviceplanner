...
$q = $db->prepare("ALTER TABLE `{$db->getPrefix()}days`
    ADD COLUMN `multichoice_lesson1` varchar(64) default NULL,
    ADD COLUMN `multichoice_lesson2` varchar(64) default NULL,
    ADD COLUMN `multichoice_gospel` varchar(64) default NULL,
    ADD COLUMN `multichoice_psalm` varchar(64) default NULL,
    ADD COLUMN `multichoice_introit` text default NULL,
    ADD COLUMN `multichoice_gradual` text default NULL");
$q->execute() or die(array_pop($q->errorInfo()));
$rm[] = "Added multichoice propers columns to days table";
$db->commit();
