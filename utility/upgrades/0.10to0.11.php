<? /* Upgrade from version 0.10 to 0.11
...
if ("0.10." != substr($oldversion, 0, 5).'.') {
    die("Can't upgrade from 0.10.x, since the current db version is {$oldversion}.");
}

$options = new Configfile("./options.ini", true, true, true);
$options->set('modifyorder', 'All');
$options->save();
unset($options);
